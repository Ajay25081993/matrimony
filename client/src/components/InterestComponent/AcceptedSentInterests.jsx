...
const AcceptedSentInterests = ({ loggedInUserData, acceptedSentInterests }) => {
  ...
  //         {
  //           receiver_id: loggedInUserData.id,
  //           receiver_id: id,
  //         }
  //       );
  ...
  //       );
  //       const pendingInterests = allInterestResponse.data?.filter(
  //         (interest) => interest.status == "pending"
  //       );
  //       console.log(pendingInterests);

  //       setAllInterests(pendingInterests);
  //     } catch (error) {
  //       console.log(error);
  //     }
  //   };
  ...
  return (
    <div className="w-2xl">
      ...
      <p className=" text-2xl font-semibold">
        {acceptedSentInterests?.length > 0
          ? `Interests that were accepted (${acceptedSentInterests.length})`
          : "No interests were accepted"}
      </p>
      {acceptedSentInterests?.length > 0 ? (
        <div>
          {acceptedSentInterests?.map((data, index) => {
            return (
              <div
                key={index}
                className="mt-5 border-1 border-gray-300 rounded-lg p-4 relative flex w-full space-x-4"
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-col mt-15 space-y-2">
                    <p className="font-semibold">
                     {data.receiver.gender === "Male" ? "He" : "She"} accepted your interest
        ...
        <div className="pt-20 space-y-3 w-full flex items-center justify-center flex-col">
          ...
          <p className="font-bold">You have no accepted interests yet</p>
...
export default AcceptedSentInterests;
