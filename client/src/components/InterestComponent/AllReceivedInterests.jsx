...
const AllReceivedInterests = ({
  ...
  const connect = (id) => {
    const connectResponse = axiosInstance.post(`${API_URLS.CONNECT}/${id}`);
    ...
    <div className="w-2xl">
      ...
      <p className=" text-2xl font-semibold">
        {receivedInterests?.length > 0
          ? `All interests received (${receivedInterests.length})`
          : "No interests received"}
      </p>
      {receivedInterests?.length > 0 ? (
        <div>
          {receivedInterests?.map((data, index) => {
            return (
              <div
                key={index}
                ...
                <div className="space-y-1">
                  ...
                  {data.status === "accepted" && (
                    <div className="flex flex-col mt-15 space-y-2">
                      <p className="font-semibold">
                        You accepted{" "}
                        {data.receiver.gender === "Male" ? "his" : "her"}{" "}
                        interest on {customDate(data.updated_at)}
                      </p>
                  ...
                  {data.status === "pending" && (
                    <div className="flex flex-col mt-9 space-y-1">
                      <p className="font-semibold">
                        {data.sender.gender === "Male" ? "He" : "She"} sent you
                        an interest {customDate(data.created_at)}
                      </p>
                      <p className="text-sm">
                  ...
                  {data.status === "declined" && (
                    <div className="mt-15 flex flex-col gap-2">
                      <p className="font-semibold">
                        You declined{" "}
                        {data.receiver.gender === "Male" ? "him" : "her"}{" "}
                        interest on {customDate(data.updated_at)}
                      </p>
                      <button
                        onClick={() => {
                        ...
                        className=" px-6 w-50 rounded-full text-[#a80ca3] hover:bg-[#ffd0fd] border border-[#9f139a] cursor-pointer flex gap-2 items-center"
                      >
                        <i className="ri-thumb-up-fill text-xl  "></i>
                        <span>Accept Interset</span>
                      </button>
        ...
        <div className="pt-20 space-y-3 w-full flex items-center justify-center flex-col">
          ...
          <p className="font-bold">You have not received any interests yet</p>
...
export default AllReceivedInterests;
