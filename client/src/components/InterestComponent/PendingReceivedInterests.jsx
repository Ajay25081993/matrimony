...
const PendingReceivedInterests = ({
  loggedInUserData,
  ...
  //     }
  //   };
  const acceptInterest = async (id,senderId) => {
    const statusChangeResponse = await axiosInstance.put(
      `${API_URLS.UPDATE_INTERESTS_STATUS}/${id}`,
      { status: "accepted" }
    );
    connect(senderId)
    fetchInterests();
  ...
  const connect = (id) => {
    const connectResponse = axiosInstance.post(`${API_URLS.CONNECT_USER}/${id}`);
  ...
  //       );
  //       const pendingReceivedInterests = allInterestResponse.data?.filter(
  //         (interest) => interest.status == "pending"
  //       );
  //       console.log(pendingReceivedInterests);

  //       setAllInterests(pendingReceivedInterests);
  //     } catch (error) {
  //       console.log(error);
  //     }
  //   };
    ...
    <div className="w-2xl">
      ...
      <p className=" text-2xl font-semibold">
        {pendingReceivedInterests?.length > 0
          ? `Pending interests (${pendingReceivedInterests.length})`
          : "No pending interests"}
      </p>
      {pendingReceivedInterests?.length > 0 ? (
        <div>
          {pendingReceivedInterests?.map((data, index) => {
            return (
              <div
                key={index}
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-col mt-9 space-y-1">
                    <p className="font-semibold">
                      {data.sender.gender === "Male" ? "He" : "She"} sent you an
                      interest {customDate(data.created_at)}
                    </p>
                    <p className="text-sm">
                      Accept {data.sender.gender === "Male" ? "his" : "her"}{" "}
                      interest to start a conversation
                    </p>
                    <div className=" flex gap-2">
                      <button
                        onClick={() => {
                          declineInterest(data.id);
                        }}
                        className="border-1 px-3 text-gray-700 border-gray-300 rounded-full cursor-pointer flex gap-2 items-center"
                      >
                        <i className="ri-thumb-down-line text-xl mt-2 text-gray-500"></i>
                        <span>Decline</span>
                      </button>
                      <button
                        onClick={() => {
                          acceptInterest(data.id,data.sender.id);
                        }}
                        className=" px-6 rounded-full text-[#a80ca3] hover:bg-[#ffd0fd] border border-[#9f139a] cursor-pointer flex gap-2 items-center"
                      >
        ...
        <div className="pt-20 space-y-3 w-full flex items-center justify-center flex-col">
          ...
          <p className="font-bold">You have no pending interests yet</p>
...
export default PendingReceivedInterests;
