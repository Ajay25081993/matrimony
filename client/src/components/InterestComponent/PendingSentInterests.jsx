...
const PendingSentInterests = ({ loggedInUserData, pendingSentInterests }) => {
  ...
  //       );
  //       const pendingSentInterests = allInterestResponse.data?.filter(
  //         (interest) => interest.status == "pending"
  //       );
  //       console.log(pendingSentInterests);

  //       setAllInterests(pendingSentInterests);
  //     } catch (error) {
  //       console.log(error);
  //     }
  //   };
  ...
  return (
    <div className="w-2xl">
      ...
      <p className=" text-2xl font-semibold">
        {pendingSentInterests?.length > 0
          ? `Pending interests (${pendingSentInterests.length})`
          : "No pending interests"}
      </p>

      {pendingSentInterests?.map((data, index) => {
        return (
          <div
            key={index}
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-col mt-16 space-y-1">
                <p className="font-semibold">
                  You sent {data.receiver.gender === "Male" ? "him" : "her"} an
...
export default PendingSentInterests;
