...
const DeclinedSentInterests = ({ loggedInUserData, declinedSentInterests }) => {
  ...
  //         `${API_URLS.GET_INTERESTS}/${user.id}`
  //       );
  //       const declinedSentInterests = allInterestResponse.data?.filter(
  //         (interest) => interest.status == "pending"
  //       );
  //       console.log(declinedSentInterests);

  //       setAllInterests(declinedSentInterests);
  //     } catch (error) {
  //       console.log(error);
  //     }
  //   };
  ...
  return (
    <div className="w-2xl">
      ...
      <p className=" text-2xl font-semibold">
        {declinedSentInterests?.length > 0
          ? `Interests that were declined (${declinedSentInterests?.length})`
      ...
      {declinedSentInterests?.length > 0 ? (
        <div>
          {" "}
          {declinedSentInterests?.map((data, index) => {
            return (
              <div
                key={index}
...
export default DeclinedSentInterests;
