...
const ViewedBy = ({ loggedInUserData }) => {
  ...
  const fetchViewers = async (user) => {
    try {
      const allViewersResponse = await axiosInstance.get(
        `${API_URLS.ALL_VISITER_USER}/${user.id}`
      );
      console.log(allViewersResponse.data);

      setAllViewers(allViewersResponse.data);
    } catch (error) {
      console.log(error);
    }
  };
  useEffect(() => {
    if (loggedInUserData.id) {
      fetchLikes(loggedInUserData);
      fetchViewers(loggedInUserData);
      fetchViews(loggedInUserData);
      fetchInterests(loggedInUserData);
    }
  }, [loggedInUserData]);
  const openProfile = (id) => {
    navigateTo(`/matches/viewed-you/${id}`);
  };
  return (
    <div className="w-2xl">
      {allViewers?.length > 0 ? (
        <div>
          <p className="mb-2 text-2xl font-semibold">
            {allViewers?.length}{" "}
            {allViewers?.length > 1 ? "Profiles" : "Profile"} viewed you{" "}
          </p>

          {allViewers?.map((data, index) => {
            return (
              <div
                key={index}
              ...
              >
                <div className="relative w-60 h-55 overflow-hidden rounded-xl border-1 cursor-pointer ">
                  <img
                    onClick={() => {
                      openProfile(data.visiter.id);
                      viewUser(data.visiter.id);
                    }}
                    className="object-cover w-full"
                    src={data.visiter.profilePic}
                    alt=""
                  />
                  <div className="absolute right-0   rounded-tr-xl rounded-l-xs top-0 bg-[#000000] text-sm font-semibold text-white">
                    {allLikedUser?.some(
                      (like) => like.liked_id === data.visiter.id
                    ) ? (
                      <div
                        onClick={() => dislikeUser(data.visiter)}
                    ...
                    !allLikedUser?.some(
                      ...
                      <div
                        onClick={() => likeThisProfile(data.visiter)}
                ...
                <div className="space-y-1">
                  ...
                  >
                    <p className="font-bold">
                      {data.visiter.firstName} {data.visiter.lastName}
                    </p>{" "}
                    <p className="text-gray-500">Last seen Few hours ago </p>
                    <div className="flex space-x-2 flex-wrap">
                      <div>{data.visiter.age} yrs</div>
                      <div className="text-[#d612cf]">●</div>
                      <div> {data.visiterUserInfo.height}</div>
                      <div className="text-[#d612cf]">●</div>
                      <div>{data.visiterUserInfo.subCommunity}</div>
                      <div className="text-[#d612cf]">●</div>
                      <div>{data.visiterUserInfo.qualification}</div>
                      <div className="text-[#d612cf]">●</div>
                      <div>{data.visiterUserInfo.workAs}</div>
                      <div className="text-[#d612cf]">●</div>
                      <div>{data.visiterUserInfo.city}</div>
                    </div>
                  </div>

                  {allInterests?.some(
                    (interset) => interset.receiver_id === data.visiter.id
                  ) ? (
