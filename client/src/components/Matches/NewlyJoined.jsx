...
const NewlyJoined = ({ otherUserData, loggedInUserData }) => {
  ...
  const recentMatches = otherUserData.filter(
    (data) => getTimeDifference(data.createdAt) < 30
  );
  ...
  const openProfile = (id) => {
    navigateTo(`/matches/newly-joined/${id}`);
  };
      ...
      {recentMatches?.length > 0 ? (
        <div>
          <p className="mb-2 text-2xl font-semibold">
            ...
            {recentMatches.length > 1 ? "Matches" : "Match"} who joined in the
            last 30 days{" "}
          </p>
          {recentMatches.map((data, index) => {
            return (
              <div
                key={index}
