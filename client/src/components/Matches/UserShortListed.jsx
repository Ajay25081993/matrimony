...
const UserShortListed = ({ loggedInUserData }) => {
  ...
  const openProfile = (id) => {
    navigateTo(`/matches/liked-by-you/${id}`);
  };
      ...
      {allLikedUser?.length > 0 ? (
        <div>
          <p className="mb-2 text-2xl font-semibold">
            {allLikedUser?.length}{" "}
            {allLikedUser?.length > 1 ? " Profiles" : "Profile"} you liked{" "}
          </p>
          {allLikedUser?.map((data, index) => {
            return (
              <div
                key={index}
              ...
              >
                <div className="relative w-60 h-55 overflow-hidden rounded-xl border-1 cursor-pointer ">
                  <img
                    onClick={() => {
                      openProfile(data.likedUser.id);
                      viewUser(data.likedUser.id);
                    }}
                    className="object-cover w-full"
                    src={data.likedUser.profilePic}
                    alt=""
                  />
                  <div className="absolute right-0 rounded-tr-xl rounded-l-xs top-0 bg-[#000000] text-sm font-semibold text-white">
                    <div
                      onClick={() => dislikeUser(data.likedUser)}
                ...
                <div className="space-y-1">
                  ...
                  >
                    <p>You've liked her on {customDate(data.createdAt)}</p>
                    <p className="font-bold">
                      {data.likedUser.firstName} {data.likedUser.lastName}
                    </p>{" "}
                    <p className="text-gray-500">Last seen Few hours ago </p>
                    <div className="flex space-x-2 flex-wrap">
                      <div>{data.likedUser.age} yrs</div>
                      <span className="text-[#d612cf]">●</span>
                      <div> {data.likedUserInfo.height}</div>
                      <span className="text-[#d612cf]">●</span>
                      <div>{data.likedUserInfo.subCommunity}</div>
                      <span className="text-[#d612cf]">●</span>
                      <div>{data.likedUserInfo.qualification}</div>
                      <span className="text-[#d612cf]">●</span>
                      <div>{data.likedUserInfo.workAs}</div>
                      <span className="text-[#d612cf]">●</span>
                      <div>{data.likedUserInfo.city}</div>
                    </div>
                  </div>

                  {allInterests?.some(
                    (interset) => interset.receiver_id === data.likedUser.id
                  ) ? (
                    ...
                    <div className="mt-20 flex gap-2">
                      ...
                      <button
                        onClick={() => {
                          sendInterest(data.likedUser.id);
                        }}
                        className=" px-6 rounded-full text-white hover:bg-[#81007c] bg-[#9f139a] cursor-pointer flex gap-2 items-center"
                      >
        ...
        <div className="pt-20 space-y-3 w-full flex items-center justify-center flex-col">
          ...
          <p>You have liked no profile yet</p>
